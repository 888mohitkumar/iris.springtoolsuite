...
@NamedQueries
(value ={
		@NamedQuery(name="Person.getbyPersonLastName",query="select p from Person  p where p.lastName=?1"),
		@NamedQuery(name="Person.getbyPersonFirstNameAndEmail",query="select p from Person  p where p.firstName=?1 and p.email=?2")
})
